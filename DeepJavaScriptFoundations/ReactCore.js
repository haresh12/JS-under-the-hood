 ...
 *  call apply and bind : 
 *  
 *  SEE AT THIS TIME IT MAY BE HARD TO REMEMBER EVERYTHING BUT THIS ONE SIMPLE SUPPOSE YOU HAVE
 *  50 STUDENT OBJECTS AND IN THAT 50 STUDENTS OBJECT NOW WE JUST WANTED TO PRINT THERE FIRST AND
 *  LAST NAME SO IN THAT CASE FOR ALL 50 STUDENT WE SHOULD NOT CREATE printName() METHOD RATHER 
 *  CREATE IN ONE AND THEN OTHER CAN BORROW THAT METHODS
 * 
 *   HARESH WHY WILL YOU USE CALL METHOD SO BASICALLY I WILL USE CALL METHOD TO BORROW SOME 
 *   METHODS FROM OTHER OBJECTS
 * 
 *   LET SEE SIMPLE EXAMPLE OF CALL
 */
...
/// I WANTED TO BORROW METHOD FROM  STUDENT RATHER THEN CREATING ONE

student.print.call(student2); // HERE PRINT METHOD WILL BE CALLED AND REFERANCE WILL BE STUDENT2

/**
 *  NOW YOU SEE THAT FIRST ARGUMENT THAT YOU ARE PASSING THAT WILL ALWAYS GOING TO BE REFERANCE TO THE
 *  FUNCTION AND SECOND ARGUMENT WILL BE ANY ARGUMENT YOU WANTED TO PASS INTO FUNCTION
 */

/**
 *  SEE IN FUTURE WE WILL GO MUCH MORE DEEP INTO THAT THIS CONCEPT BUT FOR YOU ALL YOU HAVE TO UNDERSTAND
 *  THIS WE USE CALL APPLY AND BIND JUST TO BORROW METHODS FROM ABOTHER OBJECT
 *  
 *  ALSO WHEN YOU BORROW METHOD THE FIRST ARGUMENT ALWAYS GOING TO BE REFERENCE AND SECOND ARGUMEBT
 *  WITLL BE THE ARGUMENTS THAT YOU WANTED TO PASS IN FUNCTIONS
 */


/**
 *  NOW HARESH CAN YOU TELL ME THE MAJOR DIFFERENCE BETWEEN CALL AND APPLY
 * 
 *  SO TO BE REALLY THERE IS NO MAJOR DIFFERANCE BETWEEN BOTH ITS JUST LIKE HOW YOU WANT TO PASS
 *  THE ARGUMENTS
 * 
 *  IN CALL () // FIRST REFERANCE AND THEN ARGUMENT
 *  IN APPLY () // FIRST REFERANCE AND THEN ARGUMENT BUT YOU HAVE TO PASS ARGUMENTS AS ARRAY
 * 
 *  SO AT ONCE PLACE YOU ARE PASSING ARGUMENT AS ARRAY
 */

/**
 *  BIND IS MORE CONFUSING : IT WILL RETURN US METHOD AND ATTACH TO OBJECT THAT WE CAN CALL LATERX  
 */

/**
 *  Curring :  We can do function curring using bind method see lets understand basic first
 * 
 *  WHAT BIND METHOD DOES IS JUST RETURN THE NEW FUNCTION RATHER THEN GIVEN OUT THE DIRECT OUTPUT
 *  NOW SUPPOSE YOU HAVE ONE METHOD WITH NAME MULTIPLE AND YOU WANTED TO CREATE N NUMBER OF DIFFERNT
 *  METHODS FROM THAT LIKE MULTIPLY BY 2 MULTIPLY BY 3 AND 4 AND ALL NOW IN THAT CASE OWE ACN USING FUNCTION 
 *  CURRENT WHERE IN INITAL FUNCTION YOU WANTED TO PASS THE PARAMETER THAT WILL BE USED ALMOST EVERY WNHERE
 *  AND NEXT FUNCTIONS THAT YOU WILL CREATE WITH USING BIND IN THAT YOU WILL ALREADY HAVE 5 SET UP AS
 *  
 *  PRESET PARAMETER AND IF YOU WANT TO USE MULTIPLY BY 2 METHOD THEN JUST NEED TO PASS 2 IN THAT BIND
 *  METHOD AS ARGUMENT
 * 
 *  TO BE REALLY WE HARDLY USE CURRING IN OUR DAY TO DAY CODING SO JUST REMEMBER THIS THAT CURRING CONCEPT
 *  IT SELF IS POSSIBLE JUST BECAUSE OF bind METHOD 
 * 
 *  AND ONLY BENIFIT OF CUURING IS THAT YOU HAVE SOME PRESET ARGUMENT AND LATER YOU DON'T HAVE TO PASS
 *  THOSE SAME ARGUMENT AGAIN AND AGAIN
 * 
 *  ONE OF THE BEST EXAMPLE OF CURRING COULD BE INTERSEPER OF AXIOS WHICH HELPS US TO PASS PASS AUTHORIZATION
 *  TOKEN EVERYWHERE THAT COULD BE REAL WORLD EXAMPLE
 * 
 *  THIS THING ALSO POSSIBLE WITH CLOSURE (AND THIS MAKE MORE SENSE BECAUSE CLOUSURE CAN HOLD THE 
 ...
 * 
 *  SEE FIRST THING WHEN YOU WANT TO INVOKE FUNCTION IMMIDATLY AFTER CREATION THAT CONCEPT IT SELF
 *  IS KNOW IS IIFE AND MUST UNDERSTAND THIS THAT WE USE THIS CONCEPT TO MAKE SURE THAT WE DON'T
 *  POLLUTE OR GLOBAL SCOPE ANY SCACE
 * 
 *  (function hareshCallMeImmidatliy() {
 *   console.log("PLEASE CALL ME IMMIDATLIY")
 *  })(); // OK NO ISSUE HARESH AS SOON AS I GET INTO THE FILE I WILL GET CALLED
 * 
 *  IN VIDEO I SEE WE USE FOR LIKE DATABASE CONNECION AND ALL BUT THING ABOUT IT CAN'T WE DO IT WITH
 *  useEffect but can't AND WHAT IF useEffect it self is concept of IIFE
 */


// SIMPLEST EXAMPLE OF IIFE 

// Normal function
function canYouCallMe() {
  console.log("HARESH THIS SIDE");
}

// IIFE 
(function canYouCallMeImmidatly() {
  console.log("THIS IS WHAT IT IS I WILL CALL YOU FOR SURE")
})();

// GENERATOR FUNCTION

function * helloGenerator() {
  // BECAUSE YOU HAVE ADDED * IN FUNCTION NOW I BECOME GENRATOR FUNCTION AND I HAVE SPECIAL POWER
  // AND WITH THAT I ALSO GOT ACCESS OF  Yield

  yield 1;  // IS SIMPLE WORD YIELD IS LIKE REURN FOR NORMAL FUNCTION
  yield 2;  // SEE THE POWER WITHOUT WRITTING RETURN YOU ARE RETURING VALUE FROM FUNCTION
  yield 3; 

}

// next() , throw() and return(); next will return basically two values its like 
// {value : "something" , done : false}


// CALL APPLY BIND : SHARE THE METHODS OF OBJECT IN CALL AND APPLY ONLY DIFFERENACE IS THE WAY WE PASS
// THE ARGUMENT AND IN BIND IT DOES NOT RETURN VALUE RATHER IT RETURN FUNCTION IT SELF


/**
 *  senitization of your input using yup
 *  senitaization of your HTML using senitize html
 *  JSX ALSO DO LOTS OF THINGS FOR YOU
 *  ERROR HANDLING
 *  MMKV
 *  ENCRYPTION AND DECRPTION
 *  AUTHORIZATION AND EXPIRE TOKEN
 *  CROSS SITE FROGERY ATTECK UNDERSTAND YOU REST API FOR THAT
 */
...
/**
 *  Density pixel vs Pixel now i have clear idea what that mean and how does it works
 */

/**
 * BECAUSE OF BIND ONLY CURRING IS POSSIBLE AND BECAUSE OF FUNCTIONS ALSO
 */


/**
 *  BIG THING TO NOT THAT FIBER IS THE ARCHITECTRE THAT IS USED FOR REACT
 *  WHERE HARMES IS JS ENGINE THAT IS SPECIALLY BUILD FOR REACT NATIVE
 */